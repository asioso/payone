...
<?php } elseif (in_array($state, [\PayoneBundle\Ecommerce\PaymentManager\BsPayone::PAYMENT_RETURN_STATE_FAILURE, \PayoneBundle\Ecommerce\PaymentManager\BsPayone::PAYMENT_RETURN_STATE_CANCEL])) { ?>
    ...
    <?php if ($state == \PayoneBundle\Ecommerce\PaymentManager\BsPayone::PAYMENT_RETURN_STATE_FAILURE) { ?>
        <?php
        ...
        if ($lastPaymentInfoEntry && $lastPaymentInfoEntry->getPaymentState() == \Pimcore\Bundle\EcommerceFrameworkBundle\PaymentManager\IStatus::STATUS_CANCELLED) {
            $seamlessResponse = \PayoneBundle\Ecommerce\PaymentManager\BsPayone::extractSeamlessResponse($lastPaymentInfoEntry);
